<!--Vista relacionada a las fichas rechazadas por el administrador, sobre Gastos Corrientes.-->
<?php
session_start();
...
WHERE id_GC = $identificador_FGC";
$mostrar = mysqli_query($conexion1, $sql);
$datos_FGC = mysqli_fetch_assoc($mostrar);

$sql = "SELECT * FROM ficha_gastos_detalle where id_fichas_GC = $identificador_FGC";
$mostrar2 = mysqli_query($conexion1, $sql);

$query = "SELECT SUM(total_gasto_GC) AS total FROM ficha_gastos_detalle WHERE id_fichas_GC = $identificador_FGC ";
$result = mysqli_query($conexion1, $query);
$row = mysqli_fetch_assoc($result);
$total_gasto_FGC = $row['total'];

$sql = "SELECT * FROM caja_comentarios_secpla WHERE id_ficha_GC = $identificador_FGC";
